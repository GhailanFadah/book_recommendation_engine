...
class BookDataLoader:
    ...
    COLS_NEEDED = ['book_id', 'book_title', 'author', 'genres', 
                   'num_ratings', 'num_reviews', 'average_rating', 'num_pages']
    ...
    def load_data(self, optimized) -> pd.DataFrame:
        """
        ...
        if optimized:
            self.data = pd.read_csv(self.filepath, usecols=self.COLS_NEEDED, dtype=self.OPT_TYPES)
        else:
    ...
    def preprocess_data(self) -> pd.DataFrame:
        ...
        self.data = self.data.dropna(subset=['average_rating', 'author', 'book_title'])
    ...
    def _parse_genres(self, genre_str: str) -> List[str]:
        ...
        gen_list = ast.literal_eval(genre_str)
    ...
    def _parse_pages(self, pages_str: str) -> float:
        ...
        try:
            ...
            if isinstance(pages_list, list) and len(pages_list) > 0 and pages_list[0] != None:
    ...
    def _calculate_popularity(self) -> pd.Series:
        ...
        max_rating = self.data['num_ratings'].max()
       

        normalized_data = self.data['num_ratings'] / max_rating 

        return normalized_data * (self.data['average_rating'] / 5.0)
    ...
    def load_and_preprocess(self, opt) -> pd.DataFrame:
        """
...
if __name__ == "__main__":
    ...
    print("Data loading WITH optimization")
    ...
    print(f"\nColumns: {data_opt.columns.tolist()}")

    print("\n**** Memory Usage ****")
    memory_opt = loader_opt.get_memory_usage()
    print("Total Memory:", memory_opt['total_memory_mb'], 'MB')
    ...
    print("Data loading WITHOUT optimization")
    loader_no_opt = BookDataLoader("book_data/Book_Details.csv")
    
    data_no_opt = loader_no_opt.load_and_preprocess(False)

    print("\n**** Data Info ****")
    print(f"Shape: {data_no_opt.shape}")
    print(f"\nColumns: {data_no_opt.columns.tolist()}")

    print("\n**** Memory Usage ****")
    memory_no_opt = loader_no_opt.get_memory_usage()
    print("Total Memory:", memory_no_opt['total_memory_mb'], 'MB')

    print("\n**** Sample Data ****")
    print(data_no_opt.head())
